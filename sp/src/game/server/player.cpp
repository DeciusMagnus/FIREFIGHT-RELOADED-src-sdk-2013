...
void GiveAmmoWeapon(CBasePlayer* pPlayer, const char* pClassname)
{
...
bool GiveAmmoForWeapon(CBasePlayer* pPlayer, const char* pParentClassname, bool isAmmoPrimary, int num)
{
	...
	if (pWeapon)
	{
		...
		if (isAmmoPrimary)
		{
			pAmmoType = pWeapon->GetPrimaryAmmoType();
		}
		else
		{
...
void GiveHealthkit(CBasePlayer* pPlayer, bool big = false)
{
...
void GiveBattery(CBasePlayer* pPlayer, bool big = false)
{
	CSingleUserRecipientFilter user(pPlayer);
	...
	MessageEnd();
	pPlayer->EmitSound(bfilter, pPlayer->entindex(), "ItemBattery.Touch");
}
...
//-----------------------------------------------------------------------------
static void FindRewardsFilesAbsoluteList(CUtlVector< CUtlString >& outAbsolutePathNames)
{
	...
	for (const char* pszFoundFile = g_pFullFileSystem->FindFirst("scripts/*.txt", &hFile); pszFoundFile && hFile != FILESYSTEM_INVALID_FIND_HANDLE; pszFoundFile = g_pFullFileSystem->FindNext(hFile))
	{
		...
		if (Q_strncmp(pszFoundFile, prefix, sizeof(prefix)) == 0)
		{
			...
			outAbsolutePathNames.AddToTail(szScriptName);
		}
	}
	...
	const char* pCMD)
{
	...
	switch (itemType)
	{
		case FR_BATTERY:
			GiveBattery(this);
			break;
		case FR_HEALTHKIT_BIG:
			GiveHealthkit(this, true);
			break;
		case FR_BATTERY_BIG:
			GiveBattery(this, true);
			break;
		case FR_WEAPON:
			unlocked = GiveNewWeapon(this, pWeaponClassname);
			break;
		case FR_AMMOWEAPON:
			GiveAmmoWeapon(this, pWeaponClassname);
			break;
		case FR_AMMO:
...
KeyValues* CBasePlayer::LoadItemData(KeyValues* pData, int count, int itemID)
{
	int itemdrop = itemID;
	...
	DevMsg("Trying to load Node ID %i\n", itemdrop);
	CFmtStr sectionName;
...
void CBasePlayer::Reward_GiveItem()
{
	...
	FindRewardsFilesAbsoluteList(findFileList);
...
void CBasePlayer::DamageEffect(float flDamage, int fDamageType)
{
	...
	m_dEffPrev = gpGlobals->curtime + sv_player_damageflash_waittime.GetFloat();
}
...
void CBasePlayer::Precache( void )
{
	...
	PrecacheScriptSound( "Player.PowerSlideStart");
	m_hssPowerSlideSound = PrecacheScriptSound( "Player.PowerSlide" );
	m_hssWallRunSound = PrecacheScriptSound( "Player.WallRunStart" );
	//Voice
	if (sv_player_voice.GetBool())
	{
...
bool CBasePlayer::ClientCommand( const CCommand &args )
{
	...
	else if (stricmp(cmd, "givemapadditems") == 0)
	{
		...
		engine->ServerCommand("sv_cheats 1; developer 1; debug_spawner_info 1; debug_spawner_disable 1\n");
		return true;
	}
