...
ConVar	sk_gotoboss_classic_onkillcount("sk_gotoboss_classic_onkillcount", "350", FCVAR_ARCHIVE);
...
void CAI_BaseNPC::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if (pAttacker->IsPlayer())
	{
		...
		if (GlobalEntity_GetState("player_inbossbattle") == GLOBAL_OFF && sk_gotoboss_ondronekill.GetBool())
		{
			...
			bool PlayerFulfillsRequirements = (!g_fr_classic.GetBool()) ?
...
void CAI_BaseNPC::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator )
{
	...
	{
		SpawnBlood(ptr->endpos, vecDir, BloodColor(), subInfo.GetDamage());
...
bool CAI_BaseNPC::ShouldFailNav( bool bMovementFailed )
{
	...
	if ( ai_vehicle_avoidance.GetBool() )
	{
		...
		if ( pEntity && pEntity->GetServerVehicle() )
		{
			...
			if ( Classify() == CLASS_PLAYER_ALLY_VITAL || Classify() == CLASS_PLAYER_NPC || Classify() == CLASS_VORTIGAUNT || IsNavigationUrgent() )
				return false;
		}
	}
