...
#define	VORTIGAUNT_HL2_LIMP_HEALTH				20
#define	VORTIGAUNT_HL2_SENTENCE_VOLUME			(float)0.35 // volume of vortigaunt_hl2 sentences
#define	VORTIGAUNT_HL2_VOL						0.35		// volume of vortigaunt_hl2 sounds
#define	VORTIGAUNT_HL2_ATTN						ATTN_NORM	// attenutation of vortigaunt_hl2 sentences
#define	VORTIGAUNT_HL2_HEAL_RECHARGE			15.0		// How long to rest between heals
#define	VORTIGAUNT_HL2_ZAP_GLOWGROW_TIME		0.5			// How long does glow last
#define	VORTIGAUNT_HL2_HEAL_GLOWGROW_TIME		1.4			// How long does glow last
#define	VORTIGAUNT_HL2_GLOWFADE_TIME			0.5			// How long does it fade
#define	VORTIGAUNT_HL2_STOMP_DIST				40
#define	VORTIGAUNT_HL2_BEAM_HURT				0
#define	VORTIGAUNT_HL2_BEAM_HEAL				1
#define	VORTIGAUNT_HL2_STOMP_TURN_OFFSET		20
...
ConVar	sk_vortigaunt_hl2_dmg_zap( "sk_vortigaunt_hl2_dmg_zap","0");

//=========================================================
// Vortigaunt_HL2 activities
//=========================================================
extern int	ACT_VORTIGAUNT_AIM;
extern int ACT_VORTIGAUNT_START_HEAL;
extern int ACT_VORTIGAUNT_HEAL_LOOP;
extern int ACT_VORTIGAUNT_END_HEAL;
extern int ACT_VORTIGAUNT_TO_ACTION;
extern int ACT_VORTIGAUNT_TO_IDLE;
...
extern int AE_VORTIGAUNT_ZAP_POWERUP;
extern int AE_VORTIGAUNT_ZAP_SHOOT;
extern int AE_VORTIGAUNT_ZAP_DONE;
extern int AE_VORTIGAUNT_HEAL_STARTGLOW;
extern int AE_VORTIGAUNT_HEAL_STARTBEAMS;
int AE_VORTIGAUNT_KICK;
int AE_VORTIGAUNT_STOMP;
extern int AE_VORTIGAUNT_HEAL_STARTSOUND;
...
// Interactions
//-----------------------------------------------------------------------------
int	g_interactionVortigaunt_HL2Stomp		= 0;
int	g_interactionVortigaunt_HL2StompFail	= 0;
int	g_interactionVortigaunt_HL2StompHit		= 0;
int	g_interactionVortigaunt_HL2Kick			= 0;
int	g_interactionVortigaunt_HL2Claw			= 0;

//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_Vortigaunt_HL2 )

	DEFINE_FIELD( m_flNextNPCThink,		FIELD_TIME),
	DEFINE_ARRAY( m_pBeam,				FIELD_EHANDLE, VORTIGAUNT_HL2_MAX_BEAMS ),
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_HL2::CheckHealPosition( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask)
	{

	case TASK_VORTIGAUNT_HL2_EXTRACT_WARMUP:
	{
		ResetIdealActivity( (Activity) ACT_VORTIGAUNT_TO_ACTION );
		break;
	}

	case TASK_VORTIGAUNT_HL2_EXTRACT:
	{
		SetActivity( (Activity) ACT_RANGE_ATTACK1 );
		break;
	}

	case TASK_VORTIGAUNT_HL2_EXTRACT_COOLDOWN:
	{
		ResetIdealActivity( (Activity)ACT_VORTIGAUNT_TO_IDLE );
		break;
	}

	case TASK_VORTIGAUNT_HL2_FIRE_EXTRACT_OUTPUT:
		{
	...
	case TASK_VORTIGAUNT_HL2_HEAL_WARMUP:
		{
	...
	case TASK_VORTIGAUNT_HL2_FACE_STOMP:
		{
			if (GetEnemy()!=NULL)
			{
				GetMotor()->SetIdealYaw( CalcIdealYaw ( GetEnemy()->GetLocalOrigin() ) + VORTIGAUNT_HL2_STOMP_TURN_OFFSET );
				SetTurnActivity(); 
			}
			else
			{
	...
	case TASK_VORTIGAUNT_HL2_STOMP_ATTACK:
		{
			m_flLastAttackTime = gpGlobals->curtime;
			ResetIdealActivity( (Activity)ACT_VORTIGAUNT_STOMP );
			break;
		}
	case TASK_VORTIGAUNT_HL2_GRENADE_KILL:
		{
			if (GetTarget() == NULL)
			{
				TaskComplete();
	...
	case TASK_VORTIGAUNT_HL2_ZAP_GRENADE_OWNER:
		{
			if (GetEnemy() == NULL)
			{
				TaskComplete();
	...
	case TASK_VORTIGAUNT_HL2_WAIT_FOR_PLAYER:
		{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	...
	case TASK_VORTIGAUNT_HL2_STOMP_ATTACK:
		{
			...
			if (GetEnemy() != NULL)
			{
				GetMotor()->SetIdealYawAndUpdate( CalcIdealYaw ( GetEnemy()->GetLocalOrigin() ) + VORTIGAUNT_HL2_STOMP_TURN_OFFSET, AI_KEEP_YAW_SPEED );
			}
	...
	case TASK_VORTIGAUNT_HL2_FACE_STOMP:
		{
			if (GetEnemy()!=NULL)
			{
				GetMotor()->SetIdealYawAndUpdate( CalcIdealYaw( GetEnemy()->GetLocalOrigin() ) + VORTIGAUNT_HL2_STOMP_TURN_OFFSET );
	...
	case TASK_VORTIGAUNT_HL2_HEAL:
		{
			...
			if (pPlayer)
			{
				...
				if ( pPlayer->ArmorValue() >= m_iCurrentRechargeGoal )
				{
					...
					m_flNextHealTime = gpGlobals->curtime + VORTIGAUNT_HL2_HEAL_RECHARGE;
					ClearBeams();
					EndHandGlow();
					TaskComplete();
...
//=========================================================
int CNPC_Vortigaunt_HL2::GetSoundInterests ( void) 
{
	return	SOUND_WORLD	|
			SOUND_COMBAT	|
			SOUND_CARCASS	|
...
//=========================================================
void CNPC_Vortigaunt_HL2::AlertSound( void )
{
	if ( GetEnemy() != NULL )
	{
...
//=========================================================
float CNPC_Vortigaunt_HL2::MaxYawSpeed ( void )
{
	switch ( GetActivity() )
	{
	case ACT_IDLE:		
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_Vortigaunt_HL2::RangeAttack1Conditions( float flDot, float flDist )
{
	if (GetEnemy() == NULL)
	{
		return( COND_NONE );
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt_HL2::MeleeAttack1Conditions ( float flDot, float flDist )
{
	if (flDist > 70)
	{
		return COND_TOO_FAR_TO_ATTACK;
...
// Output  :
//------------------------------------------------------------------------------
CBaseEntity*	CNPC_Vortigaunt_HL2::Kick( void )
{
	trace_t tr;
...
//------------------------------------------------------------------------------
// Purpose : Vortigaunt_HL2 claws at his enemy
// Input   :
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::Claw( int iAttachment)
{
	CBaseEntity *pHurt = CheckTraceHullAttack( 40, Vector(-10,-10,-10), Vector(10,10,10),sk_vortigaunt_hl2_dmg_claw.GetFloat(), DMG_SLASH );
	if ( pHurt )
	{
...
//=========================================================
void CNPC_Vortigaunt_HL2::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_VORTIGAUNT_ZAP_POWERUP )
	{
		...
		ArmBeam( -1 ,VORTIGAUNT_HL2_BEAM_HURT );
		ArmBeam( 1	,VORTIGAUNT_HL2_BEAM_HURT );
		BeamGlow();
	...
	{
		// Make hands glow
		StartHandGlow(VORTIGAUNT_HL2_BEAM_HEAL);
		return;
	}
	...
	if( pEvent->event == AE_VORTIGAUNT_KICK )
	{
		...
		if (pBCC)
		{
			...
			if ( pBCC->DispatchInteraction( g_interactionVortigaunt_HL2Kick, NULL, this ) )
			{
			...
			else
			{
				...
				CTakeDamageInfo info( this, this, sk_vortigaunt_hl2_dmg_claw.GetFloat(), DMG_CLUB );
				CalculateMeleeDamageForce( &info, forward, pHurt->GetAbsOrigin() );
				pBCC->TakeDamage( info );
			}
	...
	if ( pEvent->event == AE_VORTIGAUNT_STOMP )
	{
		...
		if ( pBCC )
		{
			...
			if (fDist > VORTIGAUNT_HL2_STOMP_DIST)
			{
				pBCC->DispatchInteraction( g_interactionVortigaunt_HL2StompFail, NULL, this );
				return;
			}
			else
			{
				pBCC->DispatchInteraction( g_interactionVortigaunt_HL2StompHit, NULL, this );
			}
			EmitSound( "NPC_Vortigaunt.Kick" );
		}
...
// Output  :
//------------------------------------------------------------------------------
bool CNPC_Vortigaunt_HL2::IsStompable(CBaseEntity *pEntity)
{
	if (!pEntity)
		return false;
	...
	if ( pBCC && pBCC->DispatchInteraction( g_interactionVortigaunt_HL2Stomp, NULL, this ) )
		return true;
...
//------------------------------------------------------------------------------
// Purpose : Translate some activites for the Vortigaunt_HL2
// Input   :
// Output  :
//------------------------------------------------------------------------------
Activity CNPC_Vortigaunt_HL2::NPC_TranslateActivity( Activity eNewActivity )
{
...
// Output  :
//------------------------------------------------------------------------------
bool CNPC_Vortigaunt_HL2::ShouldGoToIdleState( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::UpdateOnRemove( void)
{
	ClearBeams();
	ClearHandGlow();
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::Event_Killed( const CTakeDamageInfo &info )
{
	ClearBeams();
	ClearHandGlow();
...
bool CNPC_Vortigaunt_HL2::CreateBehaviors()
{
...
void CNPC_Vortigaunt_HL2::Spawn()
{
	...
	m_iLeftHandAttachment = LookupAttachment( VORTIGAUNT_HL2_LEFT_CLAW );
	m_iRightHandAttachment = LookupAttachment( VORTIGAUNT_HL2_RIGHT_CLAW );

	NPCInit();

	SetUse( &CNPC_Vortigaunt_HL2::Use );
}

//=========================================================
// Precache - precaches all resources this monster needs
//=========================================================
void CNPC_Vortigaunt_HL2::Precache()
{
...
// Init talk data
void CNPC_Vortigaunt_HL2::TalkInit()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_Vortigaunt_HL2::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
// PainSound
//=========================================================
void CNPC_Vortigaunt_HL2::PainSound ( void )
{
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr, CDmgAccumulator* pAccumulator)
{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt_HL2::TranslateSchedule( int scheduleType )
{
	int baseType;
	...
	case SCHED_RANGE_ATTACK1:
		return SCHED_VORTIGAUNT_HL2_RANGE_ATTACK;
		break;

	case SCHED_MELEE_ATTACK1:
		if (IsStompable(GetEnemy()))
		{
			return SCHED_VORTIGAUNT_HL2_STOMP_ATTACK;
		}
		else
		{
			...
			if ((GetEnemy() != NULL) && (GetEnemy()->MyNPCPointer() != NULL))
			{
				...
				GetEnemy()->MyNPCPointer()->DispatchInteraction( g_interactionVortigaunt_HL2Claw, &vClawPos, this );
			}
			return SCHED_VORTIGAUNT_HL2_MELEE_ATTACK;
		}
		break;
	...
	case SCHED_IDLE_STAND:
		{
			...
			if (baseType == SCHED_IDLE_STAND)
			{
				...
				return SCHED_VORTIGAUNT_HL2_STAND;
			}
			else
				return baseType;	
			break;
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_HL2::ShouldHealTarget( void )
{
	if ( IsInAScript() )
		return false;
	...
	if ( IsCurSchedule( SCHED_VORTIGAUNT_HL2_EXTRACT_BUGBAIT ) )
		return false;
...
//		return false;
	...
	if ( pEntity != NULL )
	{
		...
		if ( pPlayer->ArmorValue() < sk_vortigaunt_hl2_armor_charge.GetInt() )
		{
			m_iCurrentRechargeGoal = pPlayer->ArmorValue() + sk_vortigaunt_hl2_armor_charge.GetInt();
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt_HL2::SelectHealSchedule( void )
{
	...
	if ( ShouldHealTarget() )
		return SCHED_VORTIGAUNT_HL2_HEAL;
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_Vortigaunt_HL2::SelectSchedule( void )
{
	ClearBeams();
	EndHandGlow();
	...
	if ( m_bExtractingBugbait )
		return SCHED_VORTIGAUNT_HL2_EXTRACT_BUGBAIT;
	...
	case NPC_STATE_PRONE:
		{
			...
			{
				return SCHED_VORTIGAUNT_HL2_BARNACLE_CHOMP;
			}
			else
			{
				return SCHED_VORTIGAUNT_HL2_BARNACLE_HIT;
			}
		}
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_HL2::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_HL2::CanBeUsedAsAFriend( void )
{
	// We don't want to be used if we're busy
	if ( IsCurSchedule( SCHED_VORTIGAUNT_HL2_HEAL ) )
		return false;
	if ( IsCurSchedule( SCHED_VORTIGAUNT_HL2_EXTRACT_BUGBAIT ) )
		return false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::PrescheduleThink( void )
{
	if ( m_bGlowTurningOn )
	{
	...
	if ( ShouldHealTarget() )
	{
		SetCondition( COND_VORTIGAUNT_HL2_CAN_HEAL );
	}
	else
	{
		ClearCondition( COND_VORTIGAUNT_HL2_CAN_HEAL );
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::BuildScheduleTestBits( void )
{
	BaseClass::BuildScheduleTestBits();

	if ( IsCurSchedule( SCHED_IDLE_STAND ) || IsCurSchedule( SCHED_ALERT_STAND ) )
	{
		SetCustomInterruptCondition( COND_VORTIGAUNT_HL2_CAN_HEAL );
	}
...
//=========================================================
void CNPC_Vortigaunt_HL2::ArmBeam( int side, int beamType )
{
	trace_t tr;
	float flDist = 1.0;
	
	if (m_iBeams >= VORTIGAUNT_HL2_MAX_BEAMS)
		return;
	...
	if (beamType == VORTIGAUNT_HL2_BEAM_HEAL)
	{
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::DefendBeams( void )
{
	Vector vBeamStart;
	QAngle vBeamAng;
	...
	for (i=4;i<VORTIGAUNT_HL2_MAX_BEAMS;i++)
	{
		Vector vBeamPos = vBeamStart;
		vBeamPos.x += random->RandomFloat(-8,8);
		vBeamPos.y += random->RandomFloat(-8,8);
...
//------------------------------------------------------------------------------
// Purpose : Glow for when vortigaunt_hl2 punches manhack
// Input   :
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::ClawBeam( CBaseEntity *pHurt, int nNoise, int iAttachment )
{
	for (int i=0;i<2;i++)
	{
		CBeam* pBeam = CBeam::BeamCreate( "sprites/lgtning.vmt", 3.0 );
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::StartHandGlow( int beamType )
{
	...
	if (beamType == VORTIGAUNT_HL2_BEAM_HEAL)
	{
		m_fGlowChangeTime = VORTIGAUNT_HL2_HEAL_GLOWGROW_TIME;
		m_fGlowScale = 0.6f;
	}
	else 
	{
		m_fGlowChangeTime = VORTIGAUNT_HL2_ZAP_GLOWGROW_TIME;
		m_fGlowScale = 0.8f;
	}
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::EndHandGlow( void )
{
	m_bGlowTurningOn	= false;
	m_fGlowChangeTime = VORTIGAUNT_HL2_GLOWFADE_TIME;
...
//-----------------------------------------------------------------------------
// Purpose: vortigaunt_hl2 shoots at noisy body target (fixes problems in cover, etc)
//			NOTE: His weapon doesn't have any error
// Input  :
// Output :
//-----------------------------------------------------------------------------
Vector CNPC_Vortigaunt_HL2::GetShootEnemyDir( const Vector &shootOrigin, bool bNoisy )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt_HL2::IsValidEnemy( CBaseEntity *pEnemy )
{
	if ( IsRoller( pEnemy ) )
	{
...
//=========================================================
void CNPC_Vortigaunt_HL2::BeamGlow( )
{
	int b = m_iBeams * 32;
	if (b > 255)
		b = 255;
...
//=========================================================
void CNPC_Vortigaunt_HL2::WackBeam( int side, CBaseEntity *pEntity )
{
	Vector vecDest;
...
//=========================================================
void CNPC_Vortigaunt_HL2::ZapBeam( int side )
{
	...
	{
		CTakeDamageInfo dmgInfo( this, this, sk_vortigaunt_hl2_dmg_zap.GetFloat(), DMG_SHOCK );
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::HealBeam( int side )
{
	if (m_iBeams >= VORTIGAUNT_HL2_MAX_BEAMS)
		return;
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::ClearHandGlow( )
{
	if (m_pLeftHandGlow)
	{
...
//=========================================================
void CNPC_Vortigaunt_HL2::ClearBeams( )
{
	for (int i = 0; i < VORTIGAUNT_HL2_MAX_BEAMS; i++)
	{
		if (m_pBeam[i])
		{
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::InputEnableArmorRecharge( inputdata_t &data )
{
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::InputDisableArmorRecharge( inputdata_t &data )
{
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::InputChargeTarget( inputdata_t &data )
{
	...
	SetTarget( pTarget );
	m_iCurrentRechargeGoal = playerArmor + sk_vortigaunt_hl2_armor_charge.GetInt();
	if ( m_iCurrentRechargeGoal > 100 )
		m_iCurrentRechargeGoal = 100;
...
// Input  : &data - 
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt_HL2::InputExtractBugbait( inputdata_t &data )
{
	CBaseEntity *pTarget = gEntList.FindEntityByName( NULL, data.value.String(), data.pActivator );
	...
	SetSchedule( SCHED_VORTIGAUNT_HL2_EXTRACT_BUGBAIT );
}
...
AI_BEGIN_CUSTOM_NPC( npc_vortigaunt_hl2, CNPC_Vortigaunt_HL2 )

	DECLARE_USES_SCHEDULE_PROVIDER( CAI_LeadBehavior )

	DECLARE_TASK(TASK_VORTIGAUNT_HL2_HEAL_WARMUP)
	DECLARE_TASK(TASK_VORTIGAUNT_HL2_HEAL)
	DECLARE_TASK(TASK_VORTIGAUNT_HL2_FACE_STOMP)
	DECLARE_TASK(TASK_VORTIGAUNT_HL2_STOMP_ATTACK)
	DECLARE_TASK(TASK_VORTIGAUNT_HL2_GRENADE_KILL)
	DECLARE_TASK(TASK_VORTIGAUNT_HL2_ZAP_GRENADE_OWNER)
	DECLARE_TASK(TASK_VORTIGAUNT_HL2_EXTRACT)
	DECLARE_TASK(TASK_VORTIGAUNT_HL2_FIRE_EXTRACT_OUTPUT)
	DECLARE_TASK(TASK_VORTIGAUNT_HL2_WAIT_FOR_PLAYER)

	DECLARE_TASK( TASK_VORTIGAUNT_HL2_EXTRACT_WARMUP )
	DECLARE_TASK( TASK_VORTIGAUNT_HL2_EXTRACT_COOLDOWN )
	...
	DECLARE_CONDITION(COND_VORTIGAUNT_HL2_CAN_HEAL)

	DECLARE_INTERACTION( g_interactionVortigaunt_HL2Stomp )
	DECLARE_INTERACTION( g_interactionVortigaunt_HL2StompFail )
	DECLARE_INTERACTION( g_interactionVortigaunt_HL2StompHit )
	DECLARE_INTERACTION( g_interactionVortigaunt_HL2Kick )
	DECLARE_INTERACTION( g_interactionVortigaunt_HL2Claw )
	...
	(
		...
		"		TASK_STOP_MOVING					0"
		"		TASK_SET_ACTIVITY					ACTIVITY:ACT_IDLE"
		"		TASK_VORTIGAUNT_HL2_FACE_STOMP			0"
		"		TASK_VORTIGAUNT_HL2_STOMP_ATTACK		0"
		""
		"	Interrupts"
	...
	(
		...
		"	Tasks"
		"		TASK_VORTIGAUNT_HL2_GRENADE_KILL		0"
		"		TASK_WAIT							0.3"
		"		TASK_VORTIGAUNT_HL2_ZAP_GRENADE_OWNER	0"
		""
		"	Interrupts"
		"		COND_LIGHT_DAMAGE"
	);
	...
	(
		...
		"	Tasks"
		"		TASK_STOP_MOVING				0"
		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_VORTIGAUNT_HL2_STAND"
	...
	(
		...
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_VORTIGAUNT_HL2_BARNACLE_PULL"
		""
		"	Interrupts"
	);
	...
	(
		...
		"		TASK_SET_SCHEDULE			SCHEDULE:SCHED_VORTIGAUNT_HL2_BARNACLE_CHEW"
		""
		"	Interrupts"
	);
	...
	(
		...
		"		TASK_GET_PATH_TO_TARGET				0"
		"		TASK_MOVE_TO_TARGET_RANGE			128"				// Move within 128 of target ent (client)
		"		TASK_VORTIGAUNT_HL2_WAIT_FOR_PLAYER		0"
		"		TASK_SPEAK_SENTENCE					2"					// Start extracting sentence
		"		TASK_WAIT_FOR_SPEAK_FINISH			1"
		"		TASK_FACE_TARGET					0"
		"		TASK_WAIT_FOR_SPEAK_FINISH			1"
		"		TASK_VORTIGAUNT_HL2_EXTRACT_WARMUP		0"
		"		TASK_VORTIGAUNT_HL2_EXTRACT				0"
		"		TASK_VORTIGAUNT_HL2_EXTRACT_COOLDOWN	0"
		"		TASK_VORTIGAUNT_HL2_FIRE_EXTRACT_OUTPUT	0"
