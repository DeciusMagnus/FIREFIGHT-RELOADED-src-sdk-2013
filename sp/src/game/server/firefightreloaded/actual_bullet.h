...
inline void FireActualBullet(FireBulletsInfo_t &info, int iSpeed, const char* tracertype)
{
	...
	for (int i = 0; i < iShots; i++)
	{
		...
		pBullet->info = info;
		pBullet->Spawn();
		UTIL_Tracer(info.m_vecSrc, tr.endpos, info.m_pAttacker->entindex(), -1, (float)iSpeed, true, tracertype, 0);
		UTIL_Tracer(info.m_vecSrc, tr.endpos, info.m_pAttacker->entindex(), -1, (float)iSpeed, false, "GaussTracer", 0);
		//UTIL_ParticleTracer("bullet_tracer01", info.m_vecSrc, tr.endpos, info.m_pAttacker->entindex(), -1);
	}
}
