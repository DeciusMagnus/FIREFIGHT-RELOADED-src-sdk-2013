...
void CGrenadeFrag::CreateEffects( void )
{
	...
	if ( nAttachment < 1 )
	{
		// Estimate an offset if there is no fuse attachment.
		Vector mins, maxs;
		ComputeEntitySpaceHitboxSurroundingBox( &mins, &maxs );
		offset.x = offset.y = 0;
		offset.z = maxs.z - mins.z;
	}
	...
	if ( m_pMainGlow != NULL )
	{
		...
		else
		{
			...
			m_pMainGlow->SetLocalOrigin( offset );
		}
	...
	if ( m_pGlowTrail != NULL )
	{
		...
		else
		{
			...
			m_pGlowTrail->SetLocalOrigin( offset );
		}
...
void CGrenadeFrag::Detonate(void)
{
	if (m_firegrenade)
	{
		...
		for (i = 0; i < 16; i++)
		{
			...
			{
				FireSystem_StartFire(firetrace.endpos, 128, 0, 30.0f, (SF_FIRE_START_ON | SF_FIRE_SMOKELESS | SF_FIRE_NO_GLOW | SF_FIRE_BURN_ENTITIES), (CBaseEntity*)this, FIRE_NATURAL);
			}
		}
